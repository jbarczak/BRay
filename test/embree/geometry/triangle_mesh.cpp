...
namespace embree
{
  ...
  IntersectorRegistry<IntersectorStream> TriangleMesh::intersectorsStream;
  ...
  IntersectorStream* TriangleMesh::intersectorStream(std::string travName) const {
    assert(accel);
    //try {
      return intersectorsStream.get(accel->name(),travName,accel);
    //}
    //catch (std::exception&) {
    //  std::cout << "Warning: no stream intersector found (" << accel->name() << "," << travName << ")." << std::endl;
    //  return 0;
    //}
  }
}
