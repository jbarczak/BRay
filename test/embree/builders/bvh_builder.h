...
#if 1
          // We need left and right nodes to be positioned next to each other for our approximate ordering to work out. Should never decrease performance. /RB
          for (unsigned i = numChildren; i > bestChild+1; --i) {
            cprims[i] = cprims[i-1];
            cinfo[i] = cinfo[i-1];
            csplit[i] = csplit[i-1];
          }
          cprims[bestChild+1] = splitter.rprims; cinfo[bestChild+1] = splitter.rinfo; csplit[bestChild+1] = splitter.rsplit;
#else
